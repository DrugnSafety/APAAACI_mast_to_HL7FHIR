...
## 8. 맞춤 리포트 가독성 재구성 (커밋 `71f401d`)
알러젠 상세 블록이 정보를 한 덩어리로 늘어놓아 읽기 어려웠다(사용자 피드백). 읽는 순서를 강제하는 구조로 바꾸고, 문장은 짧게 쪼갰다.

| 순서 | 내용 | 형식 |
|---|---|---|
| ① 이름 + 배지 | 감작 강도 · 증상 정도 · 시기 · 실내/실외 | 칩(`code`) 한 줄 |
| ② 결론 | "노출될 때 증상이 실제로 나타나는 알러젠입니다. 침구와 실내 습도부터 관리하세요." | 굵은 두 문장 |
| ③ 지금 할 일 | 회피 수칙 앞 3개 | 번호 목록 |
| ④ 더 알아보기 | 판정 근거 · 왜 하나로 묶었나 · 어떤 알러젠인가(앞 2문장) · 노출 환경 · 교차반응 · 나머지 수칙 · 면역치료 | 회색 보조 블록(`.detail-more`) |

글쓰기 원칙(설치된 `humanizer` 스킬 기준): 한 문장에 한 가지, 쉼표 최소화, 명사 나열 대신 동사로 끝내기, "~에 대해/~를 통해" 같은 번역투 제거. 교육 문단·추적 관리·맺음말도 같은 원칙으로 다시 썼다. 회귀 테스트가 확인하는 마커(그룹 제목 1회, `중증 반응 병력`, 실내/실외 항원 구분, 음식 동급 경고 등)는 그대로다.

![리포트 알러젠 블록(재구성 후)](screenshots/2026-09-08/report-readable-sample.png)

```markdown
### 집먼지진드기(유럽·미국 두 종)
`강한 양성` `증상 🟠 중등증` `연중 (고온다습한 여름~초가을에 개체수 증가)` `🏠 실내`
**결론.** 노출될 때 증상이 실제로 나타나는 알러젠입니다. 침구와 실내 습도부터 관리하세요.
**지금 할 일**
1. 침구는 55~60℃ 이상 뜨거운 물로 주 1회 세탁
2. 진드기 차단(anti-mite) 커버로 매트리스·베개·이불 감싸기
3. 실내 습도 50% 이하로 유지(제습기·환기)
<div class="detail-more"> 더 알아보기 — 왜 이렇게 판단했나 / 왜 하나로 묶었나 / 어떤 알러젠인가 / 어디서 노출되나 / 교차반응 / 그 밖의 관리 수칙 / 💉 면역치료 가능 </div>
```

## 9. 클래식 UI 언어 선택 (커밋 `71f401d`)
클래식 UI(`/classic/`)에도 같은 언어 선택기를 붙였다. 클래식 전용 문구는 `web/i18n.js` 의 `c.*` 키로 관리하고, 공통 문구는 퀘스트 UI 키를 재사용한다. 서버 생성 콘텐츠가 한국어인 점은 동일하며 같은 안내 배너를 표시한다.
